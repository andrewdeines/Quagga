...
###opening stuff
setwd("C:/Documents and Settings/Bwana/My Documents/Dropbox/Dilemma/Bioscience3")
library(polynom)
#######################
###some functions
######################
demand.prod<-function (a,b,xh) {if (xh<=0) return(a)
	price<-a*(1-xh/b) 
	if (price<0) return (0) else return (price)}
harv<-function (a,b,c,q,xb) {if (xb<=0) return (0)
	H<-b*(1-(c/(a*q*xb)))
	if (H<0) return (0) else return (H)}  
Gr<-function (xb,r=r1,K=K1)if (xb<=0) return(0) else r*xb*(1-(xb/K))
Poly<-function (a,b,c,q,r,K) {   #this is a function to find the REAL roots of the Cubic in Eq.6
	roots<-polyroot(c( -((b*c)/(a*q)), b, -r, r/K   ))
...
YY<-list(i=c(u=0,b=.1),ii=c(u=0.5,b=.1),iii=c(u=1,b=1),iv=c(u=1,b=0.1))
social.est<-function (t,Xb,a,r,K,b,c,q,d,y,m) {#a function to find Xb: The solution makes the function=0 
	Yoko<-yoko(n=Xb,u=YY[[y]][1],b=YY[[y]][2],Y=K,M=m)
	GofX<-r*Xb*(1-(Xb/K))	
	CofX<-c/(q*Xb)
	Pofh<-demand.prod(a=a,b=b,xh= Gr(xb=Xb,r=r,K=K)) #assume equilibrium where harv-growth
...
social.int<-function (Xb,a,r,K,b,c,q,d,y,m) 
	...
	#takes the integral of the long-term revenue 
